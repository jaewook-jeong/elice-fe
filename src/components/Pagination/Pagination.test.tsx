...
test('페이지네이션에서 숫자가 클릭되면 해당 페이지 number가 onChange에서 호출된다.', () => {
...
test('페이지네이션에서 prev arrow 버튼이 클릭되면 현재 page의 이전값이 onChange에서 호출된다.', () => {
  const handleChangePage = jest.fn();
  const { getByText } = render(
...
test('페이지네이션에서 next arrow 버튼이 클릭되면 현재 page의 이전값이 onChange에서 호출된다.', () => {
  const handleChangePage = jest.fn();
  const { getByText } = render(
    <Pagination page={2} totalDataCount={50} countPerPage={10} onChangePage={handleChangePage} />,
  );
...
test('페이지네이션에서 현재 페이지가 1일때 prev arrow 버튼은 클릭되지 않는다.', () => {
  const handleChangePage = jest.fn();
  const { getByText } = render(
...
test('페이지네이션에서 현재 페이지가 마지작일때 next arrow 버튼은 클릭되지 않는다.', () => {
  const handleChangePage = jest.fn();
  const { getByText } = render(
